(function () {
    ...
    MyeventController.$inject = ['$scope','$rootScope', '$location', 'FlashService', 'ngDialog', 'EventService'];
        function MyeventController($scope, $rootScope,  $location, FlashService, ngDialog, EventService) {
            ...
            $scope.gotoPrevPage_save = function() {
                $scope.currentpage_save -= 1;
                $scope.updateevents_save($scope.events_save);
            }

            $scope.gotoNextPage_save = function() {
                $scope.currentpage_save += 1;
                $scope.updateevents_save($scope.events_save);
            }
            ...
            $scope.updateevents_save = function(eventlist) {
                var startpos = ($scope.currentpage_save - 1) * MAXEVENTPERPAGE;
                var endpos = ($scope.currentpage_save) * MAXEVENTPERPAGE;
                if (endpos > eventlist.length)
                    endpos = eventlist.length;
                for (var i = startpos; i < endpos; i++) {
                    eventlist[i].starttime = EventService.timeConverter(eventlist[i].time);
            ...
            $scope.pull_all_events = function() {
                ...
                $scope.trc_save(function(){
                    $scope.events_save = $rootScope.globals.event_save;
                    //console.log($rootScope.globals.event_attend);
                    $scope.maxpage_save = Math.ceil($scope.events_save.length/MAXEVENTPERPAGE);
                    //console.log($scope.maxpage_attend);
                    //$scope.updateevents_attend($scope.events_attend);
                    $scope.sortbytime_save();
                });
            ...
            $scope.trc_save = function(callback) {
                $.ajax({
                    type: "GET",
                    url: 'https://yakume.xyz/api/watchlist',
                    success: function(response){
                        console.log(response);
                        var events_save_num = JSON.parse(response).events;
                        $scope.maxpage_save = Math.ceil(events_attend_num.length/MAXEVENTPERPAGE);
                        $rootScope.globals.event_save = [];
                        var count = 0;
                        $.each(events_save_num, function (i, item) {
                            var mydata = $.param({
                                eventid : item
                            });
                            $.ajax({
                                ...
                                success: function(response){
                                    ...
                                    var t = JSON.parse(response);
                                    $rootScope.globals.event_save.push(t);
                                    if (count == events_save_num.length) {
                                        callback();
                                    }
                                }
                            });
            ...
            $scope.sortbytime_save = function() {
                $scope.events_save.sort(function(a,b){
                  return parseInt(a.time) - parseInt(b.time);
                });
                var currtime = new Date().getTime();
                var len = $scope.events_save.length;
                var cnt = 0;
                var index = 0;
                while(cnt < len){
                    if($scope.events_save[cnt].time >= currtime){
                        break;
                    }
                    cnt++;
                }
                while(index < cnt){
                    var temp = $scope.events_save.shift();
                    $scope.events_save.push(temp);
                    index++;
                }
                $scope.updateevents_save($scope.events_save);
            }
